...
export class EditarIdiomaComponent implements OnInit {
  ...
  ngOnInit() {
    this.subscribe_01 = this.guia_db.getGuia_Idiomas2 ().subscribe ((response: any []) => {
  ...
  eliminar (item: any) {
    ...
    if (opcion == true) {
      this.guia_db.removeGuia_Idiomas (item);
    } else {
  ...
  guardar (item: any) {
    ...
    if (name !== "") {
      ...
      this.guia_db.updateGuia_Idiomas (item)
        .then (() => {
  ...
  guardar_nuevo () {
    if (this.nuevo != "") {
      ...
      this.guia_db.addGuia_Idiomas (this.nuevo)
        .then (() => {
          this.nuevo = "";
          this.show_nuevo = false;
