...
export class EditarCentroFormacionComponent implements OnInit {
  ...
  ngOnInit() {
    this.subscribe_01 = this.guia_db.getGuia_Centro_Formacion ().subscribe ((response: any []) => {
  ...
  eliminar (item: any) {
    ...
    if (opcion == true) {
      this.guia_db.removeGuia_Centro_Formacion (item);
    } else {
  ...
  guardar (item: any) {
    ...
    if (name !== "") {
      ...
      this.guia_db.updateGuia_Centro_Formacion (item)
        .then (() => {
  ...
  guardar_nuevo () {
    if (this.nuevo != "") {
      ...
      this.guia_db.addGuia_Centro_Formacion (this.nuevo)
        .then (() => {
          this.nuevo = "";
          this.show_nuevo = false;
