...
export class EditarTiposGuiadoComponent implements OnInit {
  ...
  ngOnInit() {
    this.subscribe_01 = this.guia_db.getGuia_Tipo_Guiado2 ().subscribe ((response: any []) => {
  ...
  eliminar (item: any) {
    ...
    if (opcion == true) {
      this.guia_db.removeGuia_Tipo_Guiado (item);
    } else {
  ...
  guardar (item: any) {
    ...
    if (name !== "") {
      ...
      this.guia_db.updateGuia_Tipo_Guiado (item)
        .then (() => {
  ...
  guardar_nuevo () {
    if (this.nuevo != "") {
      ...
      this.guia_db.addGuia_Tipo_Guiado (this.nuevo)
        .then (() => {
          this.nuevo = "";
          this.show_nuevo = false;
