...
export class EditarTiposComponent implements OnInit {
  ...
  ngOnInit() {
    this.subscribe_01 = this.database.getEventosTipos ().subscribe ((response: any []) => {
  ...
  eliminar (item: any) {
    ...
    if (opcion == true) {
      this.database.removeEventosTipos (item);
    } else {
  ...
  guardar (item: any) {
    ...
    if (nombre !== "") {
      ...
      this.database.updateEventosTipos (item)
        .then (() => {
  ...
  guardar_nuevo () {
    ...
    this.database.addEventosTipos (this.nuevo)
      .then (() => {
        this.nuevo = "";
        this.show_nuevo = false;
